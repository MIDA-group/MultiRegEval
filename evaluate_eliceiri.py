...
# =============================================================================
# # deprecated old version. Turn to evaluate.py
# =============================================================================
...
#    display=None
    ...
    for f_name in tqdm(f_names):    
        ...
        if method == 'CA':  # CurveAlign
            dict_tlevel = {
                    'patch_trans0-20_rot0-5':'l1', 
                    'patch_trans20-40_rot5-10':'l2', 
                    'patch_trans40-60_rot10-15':'l3', 
                    'patch_trans60-80_rot15-20':'l4'}
            tlevel = dict_tlevel[os.path.split(data_root[:-1])[-1]]
            try:
                t = sio.loadmat(f'./shg_he_registration-master/eliceiri_4levels/HE/tform_{f_name}_{tlevel}.mat')
            except:
                continue
#            tform = np.eye(3)
#            tform[:2, :2] = np.asarray(t['t'])[:2, :2] # rotation
#            tform[:, -1] = np.asarray(t['t'])[-1, :] # translation
#            # transform the transformed patch coordinates back
#            coords_rec = skt.matrix_transform(coords_trans, tform)
...
def success_rate(method, gan_name='', preprocess='nopre', mode='b2a'):
    ...
    dfs = [pd.read_csv(csv_path) for csv_path 
           in glob(f'{ROOT_DIR}/patch_trans*-*_rot*-*/results_{method+gan_name}_{mode}_{preprocess}.csv')]
    
    whole_df = pd.concat(dfs)
    n_success = whole_df['Error'][whole_df['Error'] <= 16.68].count()
