# -*- coding: utf-8 -*-
# evaluate registration error and write into csv
import pandas as pd
import skimage.io as skio
import skimage.transform as skt
import scipy.io as sio
from tqdm import tqdm
import os, cv2, argparse
import numpy as np
from glob import glob
from sklearn.decomposition import PCA
from skimage import exposure


# self-defined functions
from utils.make_eliceiri_patches import dist_coords, tform_centred
#from mi import register_mi
from sift import register_sift
import sys
sys.path.append(os.path.abspath("./alpha_amd"))
import aamd

# %%
def tform_centred_rec(radian, translation, center):
    tform1 = skt.SimilarityTransform(translation=translation)
    tform2 = skt.SimilarityTransform(translation=center)
    tform3 = skt.SimilarityTransform(rotation=radian)
    tform4 = skt.SimilarityTransform(translation=-center)
    tform = tform4 + tform3 + tform2 + tform1
    return tform
# %%
def eliceiri_evaluate(data_root, method, gan_name='', data_root_fake=None, preprocess='nopre', mode='b2a', display=None):
#    data_root='./Datasets/Eliceiri_patches/patch_trans60-80_rot15-20/'
#    method='SIFT'
#    gan_name='p2p_A'
#    data_root_fake='./Datasets/Eliceiri_patches_fake'
#    preprocess='nopre'
#    mode='b2a'
#    display=None
    
#    # CurveAlign test
#    data_root='./Datasets/Eliceiri_patches/patch_trans60-80_rot15-20/'
#    method='CA'
#    gan_name=''
#    data_root_fake=None
#    preprocess='nopre'
#    mode='b2a'
#    display=None

    
    w=834
    o=608

    
    coords_ref = np.array(([0,0], [0,w], [w,w], [w,0]))
    centre_patch = np.array((w, w)) / 2. - 0.5
    
    dir_A = data_root + 'A/test/'
    dir_B = data_root + 'B/test/'
    
    if gan_name != '':
        assert data_root_fake, "data_root_fake must not be None when given gan_name."
        assert gan_name in ['cyc_A', 'cyc_B', 'p2p_A', 'p2p_B', 'drit_A', 'drit_B'], (
                "gan_name must be in 'cyc_A', 'cyc_B', 'p2p_A', 'p2p_B', 'drit_A', 'drit_B'")
        if '_A' in gan_name:
            dir_B = f'{data_root_fake}/{os.path.split(data_root[:-1])[-1]}/{gan_name}/'
        elif '_B' in gan_name:
            dir_A = f'{data_root_fake}/{os.path.split(data_root[:-1])[-1]}/{gan_name}/'
    
    assert mode in ['a2b', 'b2a', 'a2a', 'b2b'], "mode must be in ['a2b', 'b2a', 'a2a', 'b2b']"
    if mode=='a2b':
        dir_src = dir_A
        dir_tar = dir_B
    elif mode=='b2a':
        dir_src = dir_B
        dir_tar = dir_A
    elif mode=='a2a':
        dir_src = dir_A
        dir_tar = dir_A
    elif mode=='b2b':
        dir_src = dir_B
        dir_tar = dir_B
        
    assert preprocess in ['', 'nopre', 'PCA', 'hiseq'], "preprocess must be in ['', 'nopre', 'PCA', 'hiseq']"
    

    suffix_src = '_' + os.listdir(dir_src)[0].split('_')[-1]
    name_srcs = set([name[:-len(suffix_src)] for name in os.listdir(dir_src)])
    suffix_tar = '_' + os.listdir(dir_tar)[0].split('_')[-1]
    name_tars = set([name[:-len(suffix_tar)] for name in os.listdir(dir_tar)])
    f_names = name_srcs & name_tars
    f_names = list(f_names)
    f_names.sort()

#    df = pd.read_csv('./shg_he_registration-master/eliceiri_comir/info_test.csv', index_col='Filename')
    df = pd.read_csv('./shg_he_registration-master/eliceiri_comir/Eliceiri_Result_BothSmall.csv', index_col='Filename')
    
    if display:
        wsi_root='./Datasets/HighRes_Splits/WSI'
        cnt_disp = 0
    for f_name in tqdm(f_names):    
        # extract transformed patch coordinates
        coords_trans = df.loc[
                f_name, 
                ['X1_Trans', 'Y1_Trans', 'X2_Trans', 'Y2_Trans', 'X3_Trans', 'Y3_Trans', 'X4_Trans', 'Y4_Trans']
                ].to_numpy(dtype=float).reshape((4, 2))
        
        if method == 'CA':  # CurveAlign
            try:
                t = sio.loadmat(f'./shg_he_registration-master/eliceiri_comir/HE/tform_{f_name}.mat')
            except:
                continue
            
            # % EXP area ##################
            w = 833.
#            coords_ref = np.array(([0,0], [0,w], [w,w], [w,0]))
            coords_ref = df.loc[
                    f_name, 
                    ['X1_Ref', 'Y1_Ref', 'X2_Ref', 'Y2_Ref', 'X3_Ref', 'Y3_Ref', 'X4_Ref', 'Y4_Ref']
                    ].to_numpy(dtype=float).reshape((4, 2))
            centre_patch = np.array((w, w)) / 2.
            
#            translation=np.asarray((-14, -55))
#            radian=0.016526
#            rotmat = skt.SimilarityTransform(rotation=radian).params[:2, :2]
#            translation_rec = np.asarray(t['t'])[-1, :-1]
#            rotmat_rec = np.asarray(t['t'])[:2, :2]
#            
#            # matlab implementation
#            coords_trans = np.dot(coords_ref - centre_patch, rotmat.T) + centre_patch - np.dot(translation, rotmat.T)
#            # equivalent:
#            coords_trans = np.dot(rotmat, (coords_ref - centre_patch).T).T + centre_patch.T - np.dot(rotmat, translation.T).T
#            coords_trans = np.dot(rotmat, (coords_ref - centre_patch - translation).T).T + centre_patch.T
#            coords_trans = df.loc[
#                    f_name, 
#                    ['X1_Trans', 'Y1_Trans', 'X2_Trans', 'Y2_Trans', 'X3_Trans', 'Y3_Trans', 'X4_Trans', 'Y4_Trans']
#                    ].to_numpy().reshape((4, 2))
#            
#            # matlab implementation
#            coords_rec = np.dot(coords_trans - centre_patch + translation - translation_rec, rotmat_rec.T) + centre_patch + np.dot(translation_rec, rotmat_rec.T)
#            # equivalent:
#            coords_rec = np.dot(rotmat_rec, (coords_trans - centre_patch + translation - translation_rec).T).T + centre_patch.T + np.dot(rotmat_rec, translation_rec.T).T
#            coords_rec = np.dot(rotmat_rec, (coords_trans - centre_patch + translation).T).T + centre_patch.T
#            
            
            # % EXP area ##################
            # %
            
            translation = df.loc[f_name, ['Tx', 'Ty']].to_numpy(dtype=float)
            rot_matrix = np.asarray(t['t'])[:2, :2]
            translation_rec = np.asarray(t['t'])[-1, :-1]

            tform = np.eye(3)
            tform[:2, :2] = rot_matrix # rotation
            tform[:-1, -1] = translation_rec # translation
            tform = skt.SimilarityTransform(matrix=tform)
#            rot_radian = 

#            # transform patch coordinates
#            tform_patch = tform_centred(radian=0.016526, translation=(-14, -55), center=centre_patch)
#            coords_trans = skt.matrix_transform(coords_ref, tform_patch.params)


            # transform the transformed patch coordinates back
            tform_patch_rec = tform_centred_rec(radian=tform.rotation, translation=translation_rec, center=centre_patch)
            tform_patch_rec2 = tform_centred(radian=tform.rotation, translation=-tform_patch_rec.translation, center=centre_patch)
            # wrong implementation
#            tform_patch_rec2 = tform_centred(radian=tform.rotation, translation=translation, center=centre_patch)
            coords_rec = skt.matrix_transform(coords_trans, tform_patch_rec2.params)
        
        else:
            # PCA
            if preprocess == 'PCA':
                # load image with 3 channels
                img_src = cv2.imread(dir_src + f"{f_name}_T.{suffix_src.split('.')[-1]}")
                img_tar = cv2.imread(dir_tar + f"{f_name}_R.{suffix_tar.split('.')[-1]}")
                
                pixels_src = img_src.reshape(-1, img_src.shape[-1])
                result_src = PCA(n_components=1).fit_transform(pixels_src)
                img_src = result_src.reshape(img_src.shape[0], img_src.shape[1])
                pixels_tar = img_tar.reshape(-1, img_tar.shape[-1])
                result_tar = PCA(n_components=1).fit_transform(pixels_tar)
                img_tar = result_tar.reshape(img_tar.shape[0], img_tar.shape[1])
            else:
                # load image (w, h)
                img_src = cv2.imread(dir_src + f"{f_name}_T.{suffix_src.split('.')[-1]}", 0)
                img_tar = cv2.imread(dir_tar + f"{f_name}_R.{suffix_tar.split('.')[-1]}", 0)
            
    #        if img_src.ndim == 2:
    #            img_src = np.expand_dims(img_src, axis=-1)
    #        if img_tar.ndim == 2:
    #            img_tar = np.expand_dims(img_tar, axis=-1)
            
            # histogram equlisation
            if preprocess == 'hiseq':
    #            print(img_src.shape, img_tar.shape)
                img_src = exposure.equalize_adapthist(img_src, clip_limit=0.03)
                img_tar = exposure.equalize_adapthist(img_tar, clip_limit=0.03)
    
            if 'MI' in method:
                # register
                try:
                    img_rec, field = register_mi(img_src, img_tar)
                except:
                    continue
                rot_radian = float(field[0])
                tx = float(field[1])
                ty = float(field[2])
                # transform the transformed patch coordinates back
                tform_patch_rec = tform_centred_rec(radian=rot_radian, translation=(tx, ty), center=centre_patch)
                coords_rec = skt.matrix_transform(coords_trans, tform_patch_rec.params)
            elif 'aAMD' in method:
                if img_src.ndim == 2:
                    img_src = np.expand_dims(img_src, axis=-1)
                if img_tar.ndim == 2:
                    img_tar = np.expand_dims(img_tar, axis=-1)
                # register
                try:
                    img_rec, t = aamd.register_aamd(ref_im=img_tar, flo_im=img_src)
                except:
                    continue
                coords_rec = aamd.transform_coords(t, coords_in=coords_trans, centre_patch=centre_patch)
            elif 'SIFT' in method:
                img_src = cv2.imread(dir_src + f"{f_name}_T.{suffix_src.split('.')[-1]}", 0)
                img_tar = cv2.imread(dir_tar + f"{f_name}_R.{suffix_tar.split('.')[-1]}", 0)
                # register
                try:
                    img_match, img_rec, tform = register_sift(img_src, img_tar)
                except:
                    continue
                # TODO: check again here
                # fix the Problem: 
                #    skt.wrap applies translation first
                #    skt.matrix_transform applies rotation first
                #    the returned tform is for skt.wrap!
                rot_matrix = tform.params[:2, :2]
                trans_forcoords = np.dot(rot_matrix, (coords_trans[2] + tform.translation).T) - np.dot(rot_matrix, coords_trans[2].T)
                tform_patch_rec = skt.SimilarityTransform(rotation=tform.rotation, translation=trans_forcoords)
                # transform the transformed patch coordinates back
                coords_rec = skt.matrix_transform(coords_trans, tform.params)
        
        # calculate error
        disp_error = dist_coords(coords_rec, coords_ref)
        
        result = {
            'X1_Recover': coords_rec[0][0], 'Y1_Recover': coords_rec[0][1], 
            'X2_Recover': coords_rec[1][0], 'Y2_Recover': coords_rec[1][1], 
            'X3_Recover': coords_rec[2][0], 'Y3_Recover': coords_rec[2][1], 
            'X4_Recover': coords_rec[3][0], 'Y4_Recover': coords_rec[3][1], 
            'Error': np.mean(disp_error)}
        # update result
#        df.loc[f_name, 
#               ['X1_Recover', 'Y1_Recover', 'X2_Recover', 'Y2_Recover', 
#                'X3_Recover', 'Y3_Recover', 'X4_Recover', 'Y4_Recover', 
#                'Error']
#               ] = result
        df.loc[f_name, 'Error'] = result['Error']


        # display patch outline in original image
        if display:
            if cnt_disp < display:
#                f_nameA = f_name + '_SHG.tif'
#                imgA = skio.imread(f'{wsi_root}/test/{f_nameA}')
#                dispdirA = data_root + f'display/A/test'
#                if not os.path.exists(dispdirA):
#                    os.makedirs(dispdirA)
#                imgA = np.repeat(imgA.reshape(imgA.shape[0], imgA.shape[1], 1), 3, axis=-1)
#                imgA = cv2.polylines(imgA, pts=[(o+coords_ref).reshape((-1,1,2))], isClosed=True, color=(0,255,0), thickness=3)
#                imgA = cv2.polylines(imgA, pts=[np.int32(o+coords_trans).reshape((-1,1,2))], isClosed=True, color=(0,0,255), thickness=3)
#                skio.imsave(f'{dispdirA}/{f_name}_{method}_{mode}.tif', imgA)
                
                f_nameB = f_name + '_BF.tif'
                imgB = skio.imread(f'{wsi_root}/test/{f_nameB}')
                dispdirB = data_root + f'display/B/test'
                if not os.path.exists(dispdirB):
                    os.makedirs(dispdirB)
                imgB = cv2.polylines(imgB, pts=[(o+coords_ref).reshape((-1,1,2))], isClosed=True, color=(0,255,0), thickness=3)
                imgB = cv2.polylines(imgB, pts=[np.int32(o+coords_trans).reshape((-1,1,2))], isClosed=True, color=(0,0,255), thickness=3)
                imgB = cv2.polylines(imgB, pts=[np.int32(o+coords_rec).reshape((-1,1,2))], isClosed=True, color=(255,0,0), thickness=3)
                skio.imsave(f'{dispdirB}/{f_name}_{method+gan_name}_{mode}_{preprocess}.tif', imgB)
                cnt_disp += 1
            else:
                return

    
    df.to_csv(f'./shg_he_registration-master/eliceiri_comir/results_{method+gan_name}_{mode}_{preprocess}.csv')
    
    return

# %%
eliceiri_evaluate(
        data_root='./Datasets/Eliceiri_patches/patch_trans60-80_rot15-20/', 
        method='CA', 
        gan_name='', 
        data_root_fake=None, 
        preprocess='nopre',
        mode='b2a', 
        display=None)

# %%
if __name__ == '__main__':
    # for running from terminal
    parser = argparse.ArgumentParser(description='Evaluate alpha-AMD.')
    parser.add_argument(
            '--data_dir', '-d', 
            help="data_dir", 
            default='./Datasets/Eliceiri_patches/patch_trans60-80_rot15-20/')
    parser.add_argument(
            '--method', '-m', 
            help="registration method", 
            choices=['MI', 'SIFT', 'aAMD', 'CA'], 
            default='aAMD')
    parser.add_argument(
            '--gan', '-g', 
            help="gan method", 
            choices=['p2p_A', 'p2p_B', 'cyc_A', 'cyc_B', 'drit_A', 'drit_B'], 
            default='p2p_A')
    parser.add_argument(
            '--mode', 
            help="registration direction", 
            choices=['a2b', 'b2a', 'a2a', 'b2b'], 
            default='b2a')
    parser.add_argument(
            '--pre', 
            help="pre-processing method", 
            choices=['', 'nopre', 'PCA', 'hiseq'], 
            default='nopre')
    args = parser.parse_args()

    eliceiri_evaluate(
            data_root=args.data_dir, 
            method=args.method, 
            gan_name=args.gan, 
            data_root_fake='./Datasets/Eliceiri_patches_fake', 
            preprocess=args.pre,
            mode=args.mode, 
            display=None)

#    # local serial run
#    for data_dir in glob('./Datasets/Eliceiri_patches/patch_trans*-*_rot*-*/'):
#        for gan in ['p2p_A', 'p2p_B', 'cyc_A', 'cyc_B', 'drit_A', 'drit_B']:
#            eliceiri_evaluate(
#                    data_root=data_dir, 
#                    method='SIFT', 
#                    gan_name=gan, 
#                    data_root_fake='./Datasets/Eliceiri_patches_fake', 
#                    preprocess='nopre',
#                    mode='b2a', 
#                    display=None)

# %% Print out success rate
ROOT_DIR='./shg_he_registration-master/eliceiri_comir'

def success_rate(method, gan_name='', preprocess='nopre', mode='b2a'):
    # read results
    dfs = [pd.read_csv(csv_path) for csv_path 
           in glob(f'{ROOT_DIR}/results_{method+gan_name}_{mode}_{preprocess}.csv')]
    
    whole_df = pd.concat(dfs)
    n_success = whole_df['Error'][whole_df['Error'] <= 100].count()
    rate_success = n_success / len(whole_df)
    print(f'{method+gan_name}_{preprocess}', rate_success)
    
success_rate(method='CA')
# %%
success_rate(method='MI')
for method in ['SIFT', 'aAMD']:
    for gan in ['', 'p2p_A', 'p2p_B', 'cyc_A', 'cyc_B', 'drit_A', 'drit_B']:
        success_rate(method=method, gan_name=gan)
success_rate(method='VXM', preprocess='su')
success_rate(method='VXM', preprocess='us')

